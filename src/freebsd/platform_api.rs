...
pub struct FreeBsdPlatformApi {}

impl PlatformApi for FreeBsdPlatformApi {
    fn get_position(&self) -> Result<WindowPosition, ()> {
        let active_window = self.get_active_window()?;
        Ok(active_window.position)
    }
...
impl FreeBsdPlatformApi {
    fn get_active_window_x11(&self) -> Result<ActiveWindow, ()> {
        ...
        let process_path = read_link(format!("/proc/{}/file", window_pid));
